...
c = conn.cursor()
#c.execute('''CREATE TABLE nfl(title TEXT, soldprice REAL, solddate TEXT)''')
def main():
    searchterm = 'aaron+rodgers+rookie+card'
    url = f"https://www.ebay.com/sch/i.html?_from=R40&_nkw={searchterm}&_sacat=0&rt=nc&LH_Sold=1&LH_Complete=1"
    soup = get_data(url)
    products = parse(soup)
    conn.commit()
    print('complete.')
    c.execute('''SELECT * FROM nfl''')
    results = c.fetchall()
    print(results)
...
def parse(soup):
    ...
    for item in results:
        ...
        c.execute('''INSERT INTO nfl VALUES(?,?,?)''',(temp, temp2, temp3))
